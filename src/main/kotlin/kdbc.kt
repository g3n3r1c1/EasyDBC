 ...
 * This allows for a more robust experience using SQL code within Java.
 * if you're using Kotlin, use KDBC(db) instead of JDBX.DBC(db)
 */
object JDBX{
    /**
     * this method simply returns an instance of KDBC(db) for use in Java.
     * @param[db]: this will be your database URL.
     */
    @JvmStatic
    public fun DBC(db: String): KDBC{
        return KDBC(db)
    }
}
...
/**
 * A small package that provides a host of wrapper functions for common SQL queries and commands.
 * This allows for a more robust experience using SQL code within Kotlin.
 * if you're using Java, use JDBX.DBC(db) instead of KDBC(db).
 * @param[db]: this will be your database URL.
 */
class KDBC(db: String) {
     ...
     * if you're using Java, The method "JableMod" acts as a convenient wrapper for this subclass.
     */
    inner class TableMod(Table: String) {
    ...
    /**
     * A subclass specifically designated for operations such as adding and removing rows and columns.
     * if you're using Kotlin, This method is unnecessary, simply use the "TableMod" subclass.
     */
    public fun JableMod(Name: String): TableMod{
        return this.TableMod(Name)
    }
}
//hello
